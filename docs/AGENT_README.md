...
- **What**: When the user creates or deletes a `.swift` file inside a target's directory, the extension automatically updates `project.pbxproj` (adds/removes the four required entries: `PBXBuildFile`, `PBXFileReference`, `PBXGroup` child entry, `PBXSourcesBuildPhase` entry). Files are matched by their path, never their name: moving a file under its own name keeps its entry (ids, every target membership, per-file settings), a folder without a group gets one, and deleting one target's file leaves another target's same-named file alone.

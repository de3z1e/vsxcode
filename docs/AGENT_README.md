...
- **Don't manually edit pbxproj to register or remove a Swift file or a Core Data model.** Just create or delete it in the right directory; the extension's `FileSystemWatcher` (batched, write-serialized) handles the pbxproj update. The `vsxcode.syncProjectFiles` command and activation reconcile catch what a closed window missed — Swift additions (by path, creating missing groups) plus model additions, refreshes, and removals.

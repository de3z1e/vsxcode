...
- `npm run test:pbxproj` — Run every exported pbxproj parser, plus a fixed set of pbxproj writer
  ...
  and three writer edits over local projects, printing counts and list indexes only; those goldens
...
│   │                              buildFilesFor, phasesOf, targetOfPhase; element paths: parentOf,
│   │                              resolvedPath (Xcode's source trees), groupForFolder; typed value helpers
...
│                                  comments that show it), setFileReferencePath and addGroupPath on one read;
│                                  ID generation
├── sync/
...
│   │                              scheduler with its path filter, per-path debouncer (Core Data),
│   │                              bundle-aware directory walker
│   ├── swiftFileSync.ts         — FileSystemWatcher for *.swift feeding batches decided by resolved path:
│   │                              renames and moves paired by VS Code's rename event or file identity
│   │                              (dev + ino), adds, same-name re-homes, removals, letter-case fixes, group
│   │                              creation; catch-up reconcile (add-only, by path)
...
- **Reconcile**: `vsxcode.syncProjectFiles` (and activation) catches up on changes the watchers missed. Swift files are add-only, by path: each unregistered file in a target folder is added, creating missing groups, unless a registered entry with its name is missing from disk and compiled by the same target (left for repair); Core Data models are also removed when the bundle is gone from disk, since a stale XCVersionGroup fails the build with momc's "No current version for model". Removal requires the model to be absent both at its resolved path and by name, so an unresolvable group tree is never read as a deletion.

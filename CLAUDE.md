...
│                                  removeSwiftFile, rehomeSwiftFile, setFileReferencePath and addGroupPath on
│                                  one read; ID generation
├── sync/
...
│   │                              adds, same-name re-homes, removals, letter-case fixes, group creation;
│   │                              catch-up reconcile (add-only, by path)
